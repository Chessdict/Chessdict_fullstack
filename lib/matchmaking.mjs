...
export function createMatchmaking({ onMatch }) {
  ...
  function checkForMatch() {
    ...
    // 2. Staked queues — range-based matching within each token bucket and time control
        ...
        for (let i = 0; i < players.length; i++) {
          for (let j = i + 1; j < players.length; j++) {
            if (
              ...
              amountsWithinRange(players[i].stakeAmount, players[j].stakeAmount)
            ) {
              const p1 = players[i];
              const p2 = players[j];
  ...
  function joinQueue(socketId, data) {
    ...
    if (isStaked) {
      ...
      stakedQueue.get(key).push({ socketId, userId: data.userId, token, stakeAmount, onChainGameId, timeControl });
    } else {
