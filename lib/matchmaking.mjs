...
export function createMatchmaking({ onMatch }) {
  ...
  function removeExistingEntriesForUser(userId) {
    const targetUserId = normalizeQueueUserId(userId);
    if (!targetUserId) return;

    for (let index = freeQueue.length - 1; index >= 0; index -= 1) {
      if (normalizeQueueUserId(freeQueue[index].userId) === targetUserId) {
        freeQueue.splice(index, 1);
      }
    }

    for (const [key, arr] of stakedQueue) {
      for (let index = arr.length - 1; index >= 0; index -= 1) {
        if (normalizeQueueUserId(arr[index].userId) === targetUserId) {
          arr.splice(index, 1);
        }
      }
      if (arr.length === 0) stakedQueue.delete(key);
      ...
      for (let i = 0; i < freeQueue.length; i++) {
        ...
        const matchIndex = freeQueue.findIndex(
          (candidate, candidateIndex) =>
            candidateIndex > i &&
            candidate.timeControl === p1.timeControl &&
            normalizeQueueUserId(candidate.userId) !== normalizeQueueUserId(p1.userId),
        );
        ...
        for (let i = 0; i < players.length; i++) {
          for (let j = i + 1; j < players.length; j++) {
            ...
            if (
              ...
              normalizeQueueUserId(players[i].userId) !== normalizeQueueUserId(players[j].userId) &&
              firstAmount !== null &&
              secondAmount !== null
            ) {
              const p1 = players[i];
  ...
  function joinQueue(socketId, data) {
    ...
    // One wallet should only occupy one queue slot across devices.
    // This prevents self-matches if the same user opens multiple clients.
    removeExistingEntriesForUser(data.userId);
