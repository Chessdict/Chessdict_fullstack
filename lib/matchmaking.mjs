...
export function createMatchmaking({ onMatch }) {
  ...
  // Staked players keyed by lowercase token: Map<string, [{ socketId, userId, token, stakeAmount }]>
  ...
  function checkForMatch() {
    ...
    if (freeQueue.length >= 2) {
      ...
      Promise.resolve(onMatch(p1, p2, null)).catch(error => {
        ...
        for (let i = 0; i < players.length; i++) {
          for (let j = i + 1; j < players.length; j++) {
            if (amountsWithinRange(players[i].stakeAmount, players[j].stakeAmount)) {
              ...
              Promise.resolve(onMatch(p1, p2, stakeInfo)).catch(error => {
  ...
  function joinQueue(socketId, data) {
    ...
    if (isStaked) {
      const { token, stakeAmount } = data;
      const key = (token || '').toLowerCase();
      if (!stakedQueue.has(key)) stakedQueue.set(key, []);
      stakedQueue.get(key).push({ socketId, userId: data.userId, token, stakeAmount });
    } else {
