...
// ─── Regular Game Disconnect Grace ───

const GAME_DISCONNECT_GRACE_PREFIX = "gamedisconnect:";
const GAME_DISCONNECT_GRACE_TTL = 60 * 10;

export async function setGameDisconnectGrace(roomId, graceState) {
  return safeRedisWrite(() =>
    redis.set(
      `${GAME_DISCONNECT_GRACE_PREFIX}${roomId}`,
      JSON.stringify(graceState),
      "EX",
      GAME_DISCONNECT_GRACE_TTL,
    ),
  );
}

export async function getGameDisconnectGrace(roomId) {
  return safeRedisRead(async () => {
    const raw = await redis.get(`${GAME_DISCONNECT_GRACE_PREFIX}${roomId}`);
    return raw ? JSON.parse(raw) : null;
  }, null);
}

export async function deleteGameDisconnectGrace(roomId) {
  return safeRedisWrite(() => redis.del(`${GAME_DISCONNECT_GRACE_PREFIX}${roomId}`));
}
