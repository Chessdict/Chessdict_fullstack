...
export function GameBoard() {
  ...
  const [checkFlash, setCheckFlash] = useState<
  ...
  const { playMoveSound, playGameOver, playTimeOut, playOpponentMove } =
    useChessSounds();
  ...
  const effectiveColor =
    playerColor || (gameMode === "computer" ? "white" : undefined);
  const playerTurnCode = effectiveColor
    ? effectiveColor === "black"
      ? "b"
      : "w"
    : null;
  const chessboardId = useMemo(
    () => `game-board-${roomId ?? gameMode ?? "local"}`,
    [gameMode, roomId],
  );
  const isStakedMatch =
    !!stakeToken || !!stakeAmountRaw || onChainGameId !== null;
  const canRequestRematch =
    status === "finished" && isMultiplayer && !isStakedMatch;
      ...
      for (let c = 0; c < 8; c++) {
        ...
        if (piece && piece.type === "k" && piece.color === turn) {
          ...
          return {
            [`${file}${rank}`]: {
              background:
                "radial-gradient(circle, rgba(255,0,0,0.4) 50%, transparent 100%)",
            },
          };
        }
      }
  ...
  const setActiveSelection = useCallback(
    (square: string, mode: Exclude<SelectionMode, null>) => {
  ...
  const queuePremove = useCallback(
    (move: Premove) => {
      setQueuedPremove((current) =>
        current && current.from === move.from && current.to === move.to
          ? null
          : move,
      );
      clearSelection();
  ...
  useEffect(() => {
    ...
    const tick = () => {
      const remaining = Math.max(
        0,
        Math.ceil((opponentDisconnectDeadline - Date.now()) / 1000),
      );
  ...
  //@dev here to add the sound for warning the player of low time remaining,
  // //we can adjust the threshold for low time in the isLowTime helper function and play the sound
  //  using the playLowTime function from useChessSounds when the player's time goes below the threshold
  // Low time warning sound
  const lowTimeAlertedRef = useRef<{ white: boolean; black: boolean }>({
    white: false,
    black: false,
  });
  useEffect(() => {
  ...
  }, [whiteTime, blackTime, status, gameOver, playerColor, playTimeOut]);
  ...
  useEffect(() => {
    ...
    if (winner && reason) {
      ...
      const isMultiplayer = gameMode === "online" || gameMode === "friend";
      if (isMultiplayer && socket && roomId) {
        console.log("[CLIENT] Emitting gameComplete event:", {
          roomId,
          winner,
          reason,
        });
  ...
  useEffect(() => {
    ...
    const handleGameOver = ({
      ...
      reason,
      ratings,
      settlementFailed: settFailed,
    }: {
      winner: "white" | "black" | "draw" | "opponent";
      reason: any;
      ratings?: Record<string, number>;
      settlementFailed?: boolean;
    }) => {
      console.log("[CLIENT] gameOver event received:", {
        winner,
        reason,
        ratings,
        settlementFailed: settFailed,
  ...
  const handleRequestRematch = useCallback(() => {
    if (!socket || !roomId || !canRequestRematch || rematchRequestPending)
      return;
    setRematchRequestPending(true);
  ...
  const handlePostGamePlayAgain = useCallback(() => {
    ...
    if (gameMode === "online" && !isStakedMatch) {
      router.push(
        `/play?autoQueue=online&timeControl=${Math.max(1, Math.round(initialTime / 60))}`,
      );
      return;
    }
    router.push("/play");
      ...
      for (let c = 0; c < 8; c++) {
        ...
        if (piece && piece.type === "k" && piece.color === turn) {
          ...
          setCheckFlash({
            [kingSquare]: {
              background:
                "radial-gradient(circle, rgba(255,0,0,0.6) 60%, rgba(255,0,0,0.2) 100%)",
              borderRadius: "50%",
            },
          });
  ...
  const revalidateSelection = useCallback(() => {
    ...
    const nextMode: SelectionMode =
      game.turn() === playerTurnCode ? "move" : "premove";
    const selected =
      nextMode === "move"
  ...
  const tryQueuePremove = useCallback(
  ...
  const onSquareClick = ({ square }: { square: string }) => {
    if (status !== "in-progress" || !playerTurnCode || promotionSelection)
      return;
    ...
    if (sourceSquare) {
      ...
      if (
        selectionMode === "premove" &&
        !isPlayersTurn &&
        tryQueuePremove(sourceSquare, square)
      ) {
        return;
      }
  ...
  const orientation: "white" | "black" =
    effectiveColor === "black" ? "black" : "white";
  const materialBalance = useMemo(
    () => getMaterialBalance(displayFen),
    [displayFen],
  );
  ...
  useEffect(() => {
    console.log("[CLIENT] Resign button state check:", {
      status,
      gameMode,
      roomId,
      address,
  ...
  const handleResign = useCallback(() => {
    ...
    console.log("[CLIENT] Emitting resign event:", {
      roomId,
      userId: address,
      socketConnected: socket?.connected,
    });
        ...
        <div className="flex items-center justify-between px-0.5 sm:px-1">
              ...
              <div className="flex min-w-0 items-center gap-1.5 sm:gap-2">
                <p className="truncate text-xs font-semibold text-white sm:text-sm">
                  {gameMode === "computer"
                    ? "Stockfish"
                    : opponent
                      ? opponent.address.slice(0, 6) +
                        "..." +
                        opponent.address.slice(-4)
                      : "Waiting..."}
                </p>
          ...
          <div className="flex shrink-0 items-center gap-2 sm:gap-3">
            ...
            {(() => {
              ...
              return (
                <div
                  className={`flex items-center gap-1 rounded-lg px-1.5 py-0.5 ${low ? "bg-red-500 animate-pulse" : "bg-white"}`}
                >
                  <svg
                    width="16"
                    height="16"
                    ...
                    stroke="currentColor"
                    strokeWidth="2"
                    className={low ? "text-white/80" : "text-black/60"}
                  >
                    <circle cx="12" cy="12" r="10" />
                    <polyline points="12 6 12 12 16 14" />
                  </svg>
                  <span
                    className={`text-xs font-mono tabular-nums font-semibold sm:text-sm ${low ? "text-white" : "text-black"}`}
                  >
          ...
          <div className="flex items-center gap-3 rounded-xl bg-yellow-500/10 border border-yellow-500/20 px-4 py-2.5 animate-pulse">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-yellow-400/60">
                They have {disconnectCountdown}s to reconnect or they forfeit
              </p>
            </div>
          </div>
        )}
            ...
            <div className="overflow-x-auto px-1 pb-1 pt-0.5 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
              <div className="flex min-w-max items-center gap-1.5">
                {mobileMovePairs.map((pair, index) => (
                  ...
                  >
                    <span className="mr-1 font-mono text-white/40">
                      {pair.moveNumber}.
                    </span>
                    <span>{pair.white ?? "--"}</span>
                    {pair.black ? (
                      <span className="ml-1.5">{pair.black}</span>
                    ) : null}
                  </div>
                ))}
              </div>
            ...
            {gameOver && !gameResultModalDismissed && (
              <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
                <div className="w-full max-w-sm animate-in zoom-in-95 duration-300">
                  <GlassBg
                    className="p-5 sm:p-8 text-center relative"
                    height="auto"
                  >
                    <button
                    ...
                    <div className="flex flex-col items-center gap-4 sm:gap-6">
                      <div className="relative">
                        <div
                          className={`h-16 w-16 sm:h-20 sm:w-20 rounded-full flex items-center justify-center border border-white/10 shadow-xl ${gameOver.winner === "draw" ? "bg-linear-to-br from-yellow-500/20 to-orange-500/20 shadow-yellow-500/10" : gameOver.winner === playerColor || gameOver.winner === "opponent" ? "bg-linear-to-br from-green-500/20 to-blue-500/20 shadow-green-500/10" : "bg-linear-to-br from-red-500/20 to-orange-500/20 shadow-red-500/10"}`}
                        >
                      ...
                      {ratingChange !== null && (
                        <div
                          className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-full text-xs sm:text-sm font-bold border ${ratingChange > 0 ? "bg-green-500/10 text-green-400 border-green-500/20" : ratingChange < 0 ? "bg-red-500/10 text-red-400 border-red-500/20" : "bg-yellow-500/10 text-yellow-400 border-yellow-500/20"}`}
                        >
                          {ratingCategoryLabel} rating: {player?.rating ?? 1200}{" "}
                          {ratingChange > 0
                            ? `+${ratingChange}`
                            : ratingChange === 0
                              ? "+0"
                              : ratingChange}{" "}
                          → {(player?.rating ?? 1200) + ratingChange}
                        </div>
                      )}
                        ...
                        (settlementFailed ? (
                          <div className="w-full rounded-xl bg-red-500/10 border border-red-500/20 px-4 py-3 text-center">
                            ...
                            <p className="text-xs text-red-400/70 mt-1">
                              Prize cannot be claimed right now. The server was
                              unable to settle the game on-chain. Your funds are
                              safe — please contact support to resolve this.
                            </p>
                          </div>
                        ) : (
                          <button
                            onClick={async () => {
                              ...
                              await claimPrizeSingle(
                                BigInt(onChainGameId.toString()),
                              );
                          ...
                          >
                            <div
                              className={`absolute inset-0 ${prizeClaimed ? "bg-gray-600" : "bg-linear-to-r from-amber-500 to-yellow-500 opacity-90 group-hover:opacity-100"} transition-opacity`}
                            />
