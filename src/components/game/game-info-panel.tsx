...
import { useTokenDecimals, useTokenSymbol } from "@/hooks/useChessdict";
...
function formatStakeAmountDisplay(
  amount: string | null,
  ...
  if (decimals != null && /^\d+$/.test(trimmedAmount)) {
    try {
      return Number(
        formatUnits(BigInt(trimmedAmount), decimals),
      ).toFixed(2);
    } catch {
...
export function GameInfoPanel({ isSocketConnected }: GameInfoPanelProps) {
  ...
  const [observedGameStartedAt, setObservedGameStartedAt] = useState<number | null>(null);
  ...
  const canShowMobileNavigation = canReviewGame && moves.length > 0;
  const canShowMobileChatControl = isGameActive && !isMobileChatOpen;
  const showMobileActionDock =
    !isMobileChatOpen &&
    (canShowMobileNavigation || canShowMobileChatControl || (isGameActive && ["online", "friend"].includes(gameMode ?? "")));
  ...
  const { data: stakeTokenDecimals } = useTokenDecimals(stakeToken as `0x${string}` | null);
  const { data: stakeTokenSymbol } = useTokenSymbol(stakeToken as `0x${string}` | null);
  ...
  useEffect(() => {
    if (status === "in-progress" && !observedGameStartedAt) {
      setObservedGameStartedAt(Date.now());
    }
    if (status === "waiting" || status === "matched") {
      setObservedGameStartedAt(null);
    }
  }, [observedGameStartedAt, status]);
  ...
  const gameStartedAt = moves[0]?.timestamp ?? observedGameStartedAt;
  const gameStartedLabel = gameStartedAt
    ? new Intl.DateTimeFormat("en-GB", {
      ...
      }).format(gameStartedAt)
    : null;
  const gameStartedDateLabel = gameStartedAt
    ? new Intl.DateTimeFormat("en-GB", {
        day: "2-digit",
        month: "short",
        year: "numeric",
      ...
      {activeTab === "play" && (
        <div className="flex flex-col gap-4 px-3 py-3.5 sm:px-5 sm:py-4">
          ...
          {canReviewGame && activeSubTab === "moves" && (
            <p className="hidden text-xs font-medium text-white sm:block">{opponentName}&apos;s Opening</p>
          )}

          {activeSubTab === "moves" && canReviewGame && moves.length > 0 ? (
            <div className="rounded-xl border border-white/10 bg-white/5 px-3 py-2 sm:hidden">
              <p className="text-[11px] font-medium uppercase tracking-wider text-white/45">
                Moves can be viewed above
              </p>
            </div>
          ) : null}
          ...
          {activeSubTab === "info" && canReviewGame && opponent && (
            <div className="flex flex-col gap-3">
              <h3 className="text-xs font-bold uppercase tracking-widest text-white/40">Game Info</h3>
              <div className="rounded-xl border border-white/10 bg-white/5 p-3 sm:hidden">
                <div className="grid grid-cols-2 gap-x-3 gap-y-2 text-xs">
                  <span className="text-white/45">Started</span>
                  <span className="text-right font-medium text-white">
                    {gameStartedLabel ? `${gameStartedLabel} UTC+1` : "Pending"}
                  </span>
                  <span className="text-white/45">Date</span>
                  <span className="text-right text-white/80">{gameStartedDateLabel ?? "Pending"}</span>
                  <span className="text-white/45">Time Control</span>
                  <span className="text-right text-white/80">{timeControlDisplay}</span>
                  ...
                  {isStakedMatch ? (
                    <>
                      ...
                      <span className="text-right text-white/80">
                        {stakedAmountDisplay ? `${stakedAmountDisplay} ${stakeTokenSymbol ?? ""}` : "Pending"}
                      </span>
                      <span className="text-white/45">Game ID</span>
                      <span className="break-all text-right font-mono text-[11px] text-white/80">
                        {onChainGameId !== null ? onChainGameId.toString() : "Pending"}
                      </span>
                    </>
                  ) : null}
                </div>
              ...
              <div className="hidden sm:grid grid-cols-1 gap-2 sm:grid-cols-2">
                <div className="rounded-xl border border-white/10 bg-white/5 p-3">
                  <p className="text-[11px] font-medium uppercase tracking-widest text-white/35">Game Started</p>
                  <p className="mt-1 text-sm font-semibold text-white">{gameStartedLabel ? `${gameStartedLabel} UTC+1` : "Pending"}</p>
                  <p className="mt-1 text-[11px] text-white/35">{gameStartedDateLabel ?? ""}</p>
                </div>
                <div className="rounded-xl border border-white/10 bg-white/5 p-3">
                  <p className="text-[11px] font-medium uppercase tracking-widest text-white/35">Game Type</p>
                  <p className="mt-1 text-sm font-semibold text-white">
                    {isStakedMatch ? "Staked" : "Non-staked"}
                  </p>
                  {isStakedMatch && stakedAmountDisplay ? (
                    <p className="mt-1 text-[11px] text-white/45">
                      {stakedAmountDisplay} {stakeTokenSymbol ?? ""}
                    </p>
                  ) : null}
                </div>
              </div>
              ...
              <div className="hidden sm:block rounded-xl border border-white/10 bg-white/5 p-3">
                <p className="text-[11px] font-medium uppercase tracking-widest text-white/35">Colors</p>
              ...
              {isStakedMatch ? (
                <div className="hidden sm:block rounded-xl border border-emerald-400/20 bg-emerald-400/5 p-3">
                  <p className="text-[11px] font-medium uppercase tracking-widest text-emerald-200/65">Stake Details</p>
                  <div className="mt-2 flex flex-col gap-2 text-xs text-white/80">
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <span className="text-white/50">Game ID</span>
